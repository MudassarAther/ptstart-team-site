...
<div class="card">
    ...
    <div class="card-body">
        <form id="project_create_form" method="post" action="{{ !empty($project->id)?route('project.detail.edit', ['id' => $project->id]):route('project.create', ['id' => $tribe->id]) }}">
            ...
            <!-- Location -->
            <div class="form-group">
                <label for="project_location_id">Where is the location of the project?</label>
                <select class="form-control custom-select w-25 input-project-location" id="project_location_id" name="project[location_id]" required style="width: 150px;">
                  <option value="{{ $project->location_id }}" selected="selected">{{ $project->location->name }}</option>
                </select>
            </div>
